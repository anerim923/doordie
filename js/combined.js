...
$(document).ready(function(){
    ...
    /*
    이벤트문 안쪽에서의 $(this) --> 내가 이벤트를 발생시킨 바로 그 주체.
...
/*
스크립트로  on 클래스를 붙여서 박스 보이게 하는 것과
자식 컨텐츠에 transition 모션을 동시에 걸면 모션이 안걸림.

---> 약간의 시간차를 벌어줌
1. 박스를 스크립트 .hide(), show()로 먼저 열어주고
2. 0.2초의 짧은 딜레이 시간 뒤에 클래스 on 붙여서 활성화 시킴.
...
(사용자가 약관을 체크하지 않을때){
    ...
    //isRequired, isPwd, isGender 의 갓들이 모두 true이면 회원인증 완료처리
}
